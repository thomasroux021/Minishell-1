...
int my_setenv(char **table, shell_t *shell)
{
    ...
    if (!char_isalpha(table[1][0])) {
        my_puterror("setenv: The name of the variable must begin with");
        my_puterror(" a letter\n");
        return (-1);
    } else if (!str_alphanum(table[1])){
        my_puterror("setenv: The name of the variable must contain only");
        my_puterror(" alphanumerical characters\n");
        return (-1);
    }
